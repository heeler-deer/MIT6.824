...
func mkpgfilename(name *string) string {
	return "pg-" + *name + ".txt"
}
...
func (worker *AWorker) askMapTask() *MapTaskReply {
	...
	call("Master.MapAssignTask", &args, &reply)
...
func (worker *AWorker) joinMapTask(fileId int) {
	...
	reply := MapJoinReply{}
	call("Master.MapJoinTask", &args, &reply)
	if reply.Accept {
...
func (worker *AWorker) executeMap(reply *MapTaskReply) {
	...
	log.Printf("reply.Fileid: %v", reply.Fileid)
